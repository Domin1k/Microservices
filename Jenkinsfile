pipeline {
                ...
                fileContentReplaceConfig(
                    configs: [
                        fileContentReplaceItemConfig(
                            search: '(image:).\w+./\w+.\w*.\w*:latest',
                            replace: "(image:).\w+./\w+.\w*.\w*:0.0.${env.BUILD_ID}",
                            matchCount: 1)
                        ],
                    fileEncoding: 'UTF-8',
                    filePath: './Domain-driven-design/.k8s/web-services/cart-service.yml')
                ])
  ...
  //           powershell(script: 'kubectl apply -f ./Domain-driven-design/.k8s/web-services')
  //           powershell(script: 'kubectl apply -f ./Domain-driven-design/.k8s/clients') 
